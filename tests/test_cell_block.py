...
from conway3d.conway_logic import CellBlock, CellDriver
from conway3d.types import Vec3
...
class TestDriver(CellDriver):
    ...
    def first_state(self, location: Vec3, cell_block: CellBlock) -> TestState:
        return TestState.IDENTITY
...
def test_cell_block_get_index(location: Vec3, expected: int):
    cb = CellBlock((4, 4, 4), TestDriver())
    assert cb._get_index(location) == expected
...
def test_cell_block_get_neighbor_indexes(location: Vec3,
                                         expected: tuple[int]):
    cb = CellBlock((4, 4, 4), TestDriver())
    assert cb.get_neighbor_indexes(location) == expected
...
def test_cell_block_get_neighbor_count(location: Vec3, expected: int):
    cb = CellBlock((4, 4, 4), TestDriver())
    assert cb.get_neighbor_count(location) == expected
